...
class OneStopFlightsFragment(): Fragment(), AllStopsAdapter.FlightListClickListener,OnListItemClickListener<ItinerariesDetail> {
    ...
    private var oneStopFlights= arrayListOf<ItinerariesDetail>()
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        sOneStopsFlights.observe(viewLifecycleOwner) {
            if (it.isNotEmpty()) {
                oneStopFlights.clear()
                oneStopFlights.addAll(it)
  ...
  private  fun getLeastPriceFlight1(list: java.util.ArrayList<ItinerariesDetail>): java.util.ArrayList<ItinerariesDetail> {
    ...
    override fun onItemClick(item: ItinerariesDetail, pos: Int) {
        val tempList= arrayListOf<ItinerariesDetail>()
        for ((i,value ) in oneStopFlights.withIndex()){
